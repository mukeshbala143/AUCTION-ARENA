...
export default function ReAuctionPage() {
  ...
  useEffect(() => {
    const myCurrentTeam = teams.find(t => t.user_id === user?.id);
    if (myCurrentTeam) {
  ...
  const PlayerCard = ({ compact }) => !player ? (
    <div className="flex flex-col items-center justify-center h-48 text-muted">
      ...
      <p className="font-mono text-xs tracking-widest text-center">WAITING FOR NEXT PLAYER… <br />Please Wait Auction Start with in 1 minute</p>
    </div>
  ) : (
