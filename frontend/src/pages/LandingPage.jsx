...
// Sports Cards updated to fit the dark purple/indigo palette
const SPORTS = [
  { id:'ipl', icon:'🏏', tag:'IPL Cricket', name:'Indian Premier\nLeague', color:'#8B5CF6', glow:'rgba(139,92,246,0.15)', border:'rgba(139,92,246,0.3)',
  ...
  { id:'kabaddi', icon:'🤼', tag:'Pro Kabaddi', name:'Pro Kabaddi\nLeague', color:'#3B82F6', glow:'rgba(59,130,246,0.15)', border:'rgba(59,130,246,0.3)',
  ...
  { id:'football', icon:'⚽', tag:'World Football', name:'World\nFootball', color:'#EC4899', glow:'rgba(236,72,153,0.15)', border:'rgba(236,72,153,0.3)',
...
const DONATION_TIERS = [
  ...
  { name: 'Backer', amount: 50, icon: '⭐', color: '#8B5CF6' },
  { name: 'Pro', amount: 100, icon: '🚀', color: '#3B82F6' },
  { name: 'Super Supporter', amount: 200, icon: '💜', color: '#A855F7' },
  { name: 'Champion', amount: 500, icon: '🏆', color: '#EC4899' },
  { name: 'Legend', amount: 1000, icon: '🔥', color: '#F43F5E' },
  { name: 'Papa 👤', amount: 5000, icon: '👑', color: '#8B5CF6' },
...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-bg relative overflow-x-hidden">
      ...
      {/* MASSIVE EVERVAULT STYLE GLOWS */}
      <div className="orb" style={{width:'150vw', height:'80vh', background:'radial-gradient(ellipse at center, rgba(124, 58, 237, 0.2) 0%, rgba(76, 29, 149, 0.05) 50%, transparent 70%)', top:'-10%', left:'-25%', filter:'blur(120px)'}}/>
      <div className="orb" style={{width:'120vw', height:'70vh', background:'radial-gradient(ellipse at bottom, rgba(139, 92, 246, 0.25) 0%, rgba(59, 130, 246, 0.1) 40%, transparent 70%)', bottom:'-20%', left:'-10%', filter:'blur(140px)'}}/>

      {/* NAV */}
      <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-4 md:px-10 py-5"
           style={{background:'rgba(5,5,10,0.7)',backdropFilter:'blur(24px)',borderBottom:'1px solid rgba(255,255,255,0.05)'}}>
        <span className="font-bebas text-xl sm:text-2xl tracking-[2px] sm:tracking-[4px] text-white">AUCTION<span className="text-[#8B5CF6]"> ARENA</span></span>
      ...
      <section className="relative z-10 min-h-screen flex flex-col items-center justify-center text-center px-4 sm:px-6 pt-40 md:pt-48">
        ...
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full mb-8 anim-1 max-w-full text-left sm:text-center"
          ...
          <span className="w-1.5 h-1.5 rounded-full bg-[#8B5CF6] shrink-0" style={{animation:'pulse 2s infinite'}}/>
          <span className="text-white text-[10px] sm:text-xs tracking-[1px] uppercase font-semibold truncate">
        ...
        <h1 className="font-sans font-semibold leading-[1.1] anim-2 text-white" style={{fontSize:'clamp(3.5rem,10vw,7rem)',letterSpacing:'-0.03em'}}>
          ...
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#A855F7] to-[#3B82F6]">for Everyone.</span>
        </h1>
        ...
        <div className="flex flex-col items-center mt-12 w-full anim-4-5 px-4">
          <button
            ...
            className="group flex items-center justify-center gap-3 px-6 py-3 rounded-full transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_0_20px_rgba(139,92,246,0.2)]"
        ...
        <div className="mt-20 w-full max-w-5xl mx-auto anim-5 px-4 pb-20">
          <div className="flex flex-wrap justify-center gap-8 md:gap-16 items-center opacity-50 grayscale hover:grayscale-0 transition-all duration-500">
             {/* Logo placeholders - you can replace with actual SVG text later if needed */}
                ...
                <div className="flex gap-3 mt-8">
                  {isDisabled ? (
                    ...
                    <Link to={`/create-room?sport=${s.id}`} className="flex-1 py-3 rounded-full text-center text-xs font-bold no-underline transition-all hover:brightness-110"
                          style={{background:s.color,color:'#FFFFFF'}} onClick={e=>e.stopPropagation()}>Create Room</Link>
                  )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
          {STEPS.map(([num, title, desc]) => (
            <div key={num} className="p-6 rounded-2xl relative overflow-hidden transition-transform duration-300 hover:-translate-y-1" style={{background:'rgba(255,255,255,0.02)', border:'1px solid rgba(255,255,255,0.05)'}}>
              ...
              <div className="w-8 h-8 rounded-full bg-[#8B5CF6]/20 text-[#A855F7] flex items-center justify-center font-bold mb-4 border border-[#8B5CF6]/30 text-sm">{num}</div>
              <div className="font-semibold text-sm mb-2 text-white">{title}</div>
              <div className="text-muted text-xs leading-relaxed font-medium">{desc}</div>
            </div>
          ))}
      ...
      <section className="relative z-10 max-w-7xl mx-auto px-4 md:px-8 pb-20">
        <div className="rounded-3xl p-8 sm:p-12 md:p-20 text-center relative overflow-hidden" style={{background:'linear-gradient(180deg, rgba(139,92,246,0.1) 0%, rgba(0,0,0,0) 100%)',border:'1px solid rgba(139,92,246,0.2)'}}>
      ...
      <footer className="relative z-10 border-t px-4 sm:px-10 py-8 flex items-center justify-between flex-wrap gap-6 mt-10" style={{borderColor:'rgba(255,255,255,0.05)', background:'transparent'}}>
        ...
        <div className="flex flex-col items-center sm:items-start w-full sm:w-auto order-1">
          <span className="font-bebas text-xl tracking-[4px] text-white">AUCTION<span className="text-[#8B5CF6]"> ARENA</span></span>
        ...
        <div className="flex flex-row items-center justify-center gap-8 sm:gap-12 w-full sm:w-auto order-3 sm:order-2">
          ...
          <div className="flex flex-col items-center">
            ...
            <div className="flex items-center gap-4">
              <a href="https://www.instagram.com/_itz.subrata" target="_blank" rel="noopener noreferrer" className="text-muted hover:text-[#8B5CF6] transition-colors" title="Instagram">
              ...
              <a href="https://www.linkedin.com/in/subrata-bala-89516b302" target="_blank" rel="noopener noreferrer" className="text-muted hover:text-[#8B5CF6] transition-colors" title="LinkedIn">
          ...
          <div className="flex flex-col items-center">
            ...
            <div className="flex items-center gap-4">
              <a href="https://www.instagram.com/mm__raj" target="_blank" rel="noopener noreferrer" className="text-muted hover:text-[#8B5CF6] transition-colors" title="Instagram">
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/></svg>
              </a>
              <a href="https://www.linkedin.com/in/mukeshbala143" target="_blank" rel="noopener noreferrer" className="text-muted hover:text-[#8B5CF6] transition-colors" title="LinkedIn">
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M4.98 3.5c0 1.381-1.11 2.5-2.48 2.5s-2.48-1.119-2.48-2.5c0-1.38 1.11-2.5 2.48-2.5s2.48 1.12 2.48 2.5zm.02 4.5h-5v16h5v-16zm7.982 0h-4.968v16h4.969v-8.399c0-4.67 6.029-5.052 6.029 0v8.399h4.988v-10.131c0-7.88-8.922-7.593-11.018-3.714v-2.155z"/></svg>
              </a>
            </div>
          </div>
      ...
      {activeModal && (
        <div className="fixed inset-0 z-[100] flex items-start md:items-center justify-center p-3 sm:p-4 overflow-y-auto" style={{background:'rgba(0,0,0,0.8)', backdropFilter:'blur(8px)'}}>
          ...
          <div 
            ...
            style={{background:'#0D0D14', border:'1px solid rgba(255,255,255,0.1)', boxShadow:'0 25px 50px -12px rgba(0, 0, 0, 1)'}}
          >
            ...
            {activeModal === 'donate' && (
              <div className="w-full max-w-2xl flex flex-col mx-auto mt-2">
                <div className="text-center mb-8 sm:mb-10">
                   <div className="inline-flex items-center justify-center w-14 h-14 rounded-full bg-violet-500/10 text-violet-400 mb-5 border border-violet-500/20 text-2xl">💜</div>
                ...
                <div className="space-y-5 max-w-md mx-auto w-full">
                  <div>
                    ...
                    <input type="text" placeholder="e.g. Auction Arena" className="w-full px-5 py-3.5 rounded-xl text-sm text-white outline-none focus:border-violet-500 transition-colors bg-white/5 border-white/10" style={{borderWidth: '1px'}} />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-muted mb-2">Custom Amount (₹20 min)</label>
                    <div className="relative">
                       <span className="absolute left-5 top-1/2 -translate-y-1/2 text-violet-400 font-mono">₹</span>
                       <input 
                         type="number" 
                         value={donationAmount}
                         onChange={(e) => setDonationAmount(e.target.value)}
                         min="20" 
                         className="w-full pl-9 pr-5 py-3.5 rounded-xl text-sm text-white outline-none focus:border-violet-500 transition-colors font-mono bg-white/5 border-white/10" 
                         style={{borderWidth: '1px'}} 
                       />
                    </div>
                  </div>
            ...
            {activeModal === 'privacy' && (
              <div>
                ...
                <div className="text-muted text-sm space-y-5 h-72 overflow-y-auto pr-4 custom-scrollbar font-medium leading-relaxed">
                  ...
                  <p>If you have any questions regarding this policy, please reach out to us at:<br/>📧 Email: <a href="https://mail.google.com/mail/?view=cm&fs=1&to=support.auctionarena@gmail.com" target="_blank" rel="noopener noreferrer" className="text-violet-400 hover:text-white">support.auctionarena@gmail.com</a></p>
                </div>
              </div>
            )}
            ...
            {activeModal === 'contact' && (
              <div className="flex flex-col md:flex-row gap-8 sm:gap-12 mt-2">
                ...
                <div className="relative flex-[1.2] p-6 sm:p-8 rounded-2xl" style={{background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.05)'}}>
                  ...
                  {showContactThankYou && (
                    <div className="absolute inset-0 z-20 flex items-center justify-center rounded-2xl" style={{background:'rgba(13,13,20,0.8)', backdropFilter:'blur(8px)'}}>
                  ...
                  <form onSubmit={handleContactSubmit} className="space-y-5">
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-muted mb-2">Your Name</label>
                      <input type="text" name="name" required placeholder="Jane Doe" className="w-full px-4 py-3 rounded-xl text-sm text-white outline-none focus:border-violet-500 transition-colors bg-white/5 border border-white/10" />
                    </div>
                    
                    <div>
                      <label className="block text-xs font-semibold text-muted mb-2">Email Address</label>
                      <input type="email" name="email" required placeholder="jane@example.com" className="w-full px-4 py-3 rounded-xl text-sm text-white outline-none focus:border-violet-500 transition-colors bg-white/5 border border-white/10" />
                    </div>

                    <div>
                      <label className="block text-xs font-semibold text-muted mb-2">Subject</label>
                      <input type="text" name="subject_user" required placeholder="How can we help?" className="w-full px-4 py-3 rounded-xl text-sm text-white outline-none focus:border-violet-500 transition-colors bg-white/5 border border-white/10" />
                    </div>
                    
                    <div>
                      <label className="block text-xs font-semibold text-muted mb-2">Message</label>
                      <textarea name="message" rows="3" required placeholder="Tell us more..." className="w-full px-4 py-3 rounded-xl text-sm text-white outline-none focus:border-violet-500 transition-colors resize-none custom-scrollbar bg-white/5 border border-white/10"></textarea>
                    </div>
